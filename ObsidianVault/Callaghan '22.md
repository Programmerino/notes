---
csl: http://www.zotero.org/styles/acm-sig-proceedings
bibliography: storage/Library.json
---
...
PragPub is a program ran by *The Pragmatic Bookshelf* which publishes magazines written by multiple authors every month, each with a general topic area. The creator, Michael Swaine, writes “PragPub is a monthly magazine by and for software developers \[…\] that I created in 2009”. The description of an anthology comprised of selected articles from the magazine writes that… ![[Swaine '22#Extracts#1]](Swaine & PragPub writers, 2022)

Although this description appears to be poised for software developers in general, several articles require heavy understanding of functional programming before they can be well understood (such as [[Callaghan '22|the article chosen for this analysis]]). Because of this, the target audience is likely intermediary to expert software developers interested in functional programming (Subramaniam, 2022). Since functional programming tends to be more academic than other programming paradigms, there is some overlap to be expected with the audience of the other article analyzed.
### Visuals and Length
...
### Authors
There appears to be a fixed set of PragPub authors, all of which have some level of experience with software development. The publisher's web page notes ![[ThePragmaticBookshelf '22#Extracts#1]] (The Pragmatic Bookshelf, 2022)
## Callaghan '22 Summary
Additional precision from dependent types can provably replace tests/assertions, and also allows for type-driven development (a programming paradigm). Idris is a programming language which attempts to implement dependent types practically. It then explains how to install and run Idris code. Holes are placeholder values which can have their types inferred by the compiler. The article then explains how to use the Idris proof assistant to determine suitable values for these holes based on these inferences. It then explains the *Curry-Howard correspondence,* which claims that there is an intimate relationship between propositions and types, and proofs and programs. It also illustrates the movement away from true/false and ability to prove vs inability to prove in the Idris proof checking system. Finally, it uses an example of a test, which expresses “there exists” which is changed into a “for all” when represented as an Idris dependent type. Finally, it illustrates a trivial proof written in Idris for the newly made type. As a conclusion, the author writes that he will write more about the topic in the future, talks about how this methodology has been used to verify “complex protocols and many important results from math” and advocates for the methodology.
