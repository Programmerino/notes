...
(function($, Drupal, window, document) {
  'use strict';
  Drupal.behaviors.cc_ads_blocks_not_injected = {
    attach: function(context, settings) {
      // Give doubled ads a little wrapper
      /**
       * Here's the plan. Make sure this JS comes between the injection JS and the Jquery dialog js.
       * If a modal is found, remove the dialog class from the others. Dead simple.
       */
...
/**
 * @file
 * Make sure that in teasers, there are no duplicated section titles.
 */

(function($, Drupal, window, document) {
  'use strict';
  Drupal.behaviors.cc_primary_topic_dedupe = {
    attach: function(context, settings) {
      $('.cc-listing--current-issue [class^="cc-primary-topic__name-"]').each(function(i) {
        var topicClass = $(this).attr("class").split(' ')[0];
        var nextArticle = $(this).next();
        if ($(nextArticle).hasClass(topicClass)) {
          $(nextArticle).find('.cc-listing__header').hide();
        }
      });
    }
  };
