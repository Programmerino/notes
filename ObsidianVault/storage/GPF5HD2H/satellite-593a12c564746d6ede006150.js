...
function dynamicTargeting() {
  ...
  googletag.pubads().setTargeting('meid_seg', window.meid_seg || targetingKeys.join(','));
}
        ...
        if (elementValue.indexOf(deviceType) > -1 || elementValue.indexOf('all-platforms') > -1) {
          ...
          if ($('#' + elementValue + '').length > 0 && $('#' + elementValue + '').children().length < 1) {
            window.googletag.cmd.push(function () {
              ...
              googletag.pubads().setTargeting('ad_group', Adomik.randomAdGroup());
              googletag.pubads().setTargeting('ad_h', (new Date).getUTCHours().toString());
              googletag.pubads().setTargeting('meid', window.meID || _satellite.readCookie('UID'));
              googletag.pubads().setTargeting('gs_cat', gs_channels);
              if(_satellite.getQueryParam('adtechpre') != undefined) {
                googletag.pubads().setTargeting('atp', _satellite.getQueryParam('adtechpre'));
              }
              dynamicTargeting();
              setGPTKeywords();
              getCNAContext();
              if (elementValue.indexOf('out_of_page') > -1) {
                ...
                if (elementValue.indexOf(jsonObj.adUnit) > -1) {
                  ...
                  /*window.googletag.pubads().addEventListener('slotRenderEnded', function(event) {
                    //alert(event.isEmpty);
                    if(!event.isEmpty) {
                      var cAds = window.top.document.querySelectorAll(".c-advertisement--rectangle");
                      for (i = 0; i < cAds.length; ++i) {
                       var cAdsDivs = cAds[i].querySelector("div");
                       if (cAdsDivs.innerText === undefined || cAdsDivs.innerText === "") {
                         cAds[i].style.display = "none";
                       }
                      }
                    }
        					});*/
