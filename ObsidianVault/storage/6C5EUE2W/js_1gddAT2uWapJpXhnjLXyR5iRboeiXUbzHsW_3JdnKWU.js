  ...
  Drupal.behaviors.citation = {
    attach: function (context, setting) {
      ...
      if (setting.pathPrefix == 'es/') {
        dateStr = ' ' + day + ' de ' + Drupal.t(months[month]) + ' del ' + year + '.';
      } else {
        dateStr = ' ' + months[month] + ' ' + day + ', ' + year + '.';
      }

      if (setting.pathPrefix == 'es/') {
        dateStrAbbr = Drupal.t(' Accessed ') + day + ' ' + monthsabbr[month] + ' ' + year + '.';
      } else {
        dateStrAbbr = ' Accessed ' + day + ' ' + monthsabbr[month] + ' ' + year + '.';
      }

      // set the date in citation
      $('div.citation .citation-content #selectme1 p', context).html(citationContent1 + dateStr);
