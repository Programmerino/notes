...
;
(function ($) {
  Drupal.behaviors.thehill_blocks = {
    attach: function (context, settings) {
      if (typeof settings.rba.start !== 'undefined' && typeof settings.rba.end !== 'undefined') {
        var now = Date.now();
        if (now > settings.rba.start && now < settings.rba.end) {
          var cookieName = 'thehill_rba_cookie';
          rba = $.cookie(cookieName);
          if (rba === null) {
            jQuery.cookie(cookieName+"_url", document.location.href, { expires: 0, path: "/" });
            document.location.href = settings.rba.url;
          }  
        }
      } 
      if ($('.roadblock_continue').length > 0) {
        var cookieName = 'thehill_rba_cookie_url';
        url = $.cookie(cookieName);
        if (url !== null) {
          $('.roadblock_continue').attr('href', url);
        } 
      }
      $('.roadblock_continue').click(function(e){
        e.preventDefault();
        var cookieName = 'thehill_rba_cookie_url';
        url = $.cookie(cookieName);
        if (url !== null) {
...
function thehill_rba_page() {
  var expiration = Drupal.settings.rba.expiration;
  if (typeof expiration !== 'undefined') {
    var cookie_expire = Date.now() + expiration * 1000;
    var expires = new Date(cookie_expire);
    jQuery.cookie("thehill_rba_cookie", 1, { expires: expires, path: "/" });
  }
  var timer = Drupal.settings.rba.timer;
  if (typeof timer !== 'undefined') {
    setTimeout(function(){
      jQuery('.roadblock_continue').trigger('click');
    }, timer);
  }
}
;
