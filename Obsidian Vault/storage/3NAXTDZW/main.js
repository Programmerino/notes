...
//
// });
})(this,document);;(function(win,doc,undefined){var utils=PP.utils;doc.addEventListener("DOMContentLoaded",function(){var newsletterForms=doc.querySelector(".ajax-form");newsletterForms.addEventListener("submit",function(event){event.preventDefault();var currentForm=this;var submitBtn=currentForm.querySelector("input[type=\"submit\"]");var submitVal=submitBtn.value;submitBtn.value="Submitting\u2026";submitBtn.setAttribute("disabled",true);currentForm.classList.add("submitting");var paramHolder=[];var children=this.querySelectorAll("*");for(var i=0;i<children.length;i++){if(children[i].name&&children[i].value&&children[i].type!="submit"){paramHolder.push(encodeURIComponent(children[i].name)+"="+encodeURIComponent(children[i].value))}}paramHolder.push("ajax=true");var myData=paramHolder.join("&");var url=this.action;var xhr=new XMLHttpRequest;xhr.open("POST",url,true);xhr.responseType="json";xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");xhr.onload=function(e){if(xhr.status!=200){// request failed
console.log("xhr request failed");console.log(xhr.response);//
//
// TODO: Add error message text
// TODO: Add styles for error
//
//
currentForm.classList.remove("submitting");currentForm.classList.add("error");submitBtn.value="Check email address and try again";submitBtn.removeAttribute("disabled")}else{// request succeeded
console.log("Hooray. You\u2019ve signed up for a newsletter.");console.log(xhr.response);//
//
// TODO: Add styles for success
//
//
currentForm.classList.remove("submitting");currentForm.classList.remove("error");currentForm.classList.add("success");submitBtn.value="You\u2019re signed up!";setTimeout(function(){submitBtn.removeAttribute("disabled");submitBtn.value=submitVal;currentForm.querySelector("input[name=\"email\"]").value=""},3000)}};xhr.onerror=function(e){// non-HTTP error
console.log("A non-HTTP error occurred with this form.");currentForm.classList.remove("submitting");currentForm.classList.add("error");submitBtn.value="Check email address and try again";submitBtn.removeAttribute("disabled")};xhr.send(myData)})})})(this,document);;/*
...
var imgurl=entry.img["1:1"].full;if(entry.img["1:1"].oneOne150w){imgurl=entry.img["1:1"].oneOne150w}else if(entry.img["1:1"].oneOne300w){imgurl=entry.img["1:1"].oneOne300w}else if(entry.img["1:1"].oneOne600w){imgurl=entry.img["1:1"].oneOne600w}else if(entry.img["1:1"].imgurl){imgurl=entry.img["1:1"].imgurl}if(imgurl){output+="<div class=\"lead-art\"><a class=\"aspect-1-1\" href=\""+url+"\">";output+="<img src=\""+imgurl+"\" srcset=\"";for(var isize in entry.img["1:1"]){switch(isize){case"oneOne150w":output+=entry.img["1:1"][isize]+" 150w,";break;case"oneOne300w":output+=entry.img["1:1"][isize]+" 300w,";break;case"oneOne600w":output+=entry.img["1:1"][isize]+" 600w,";}}// hack to remove trailing comma
...
output+="<div class=\"description\"><h4 class=\"hed\"><a href=\""+url+"\">"+entry.title+"</a></h4>";if(entry.hasOwnProperty("deck")&&entry.deck){output+="<p class=\"dek\">"+entry.deck+"</p>"}output+="</div></div>"});// Drop in
...
//# sourceMappingURL=main.98d4b3e8.js.map
