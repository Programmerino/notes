...
(function($, Drupal, window, document) {
  Drupal.behaviors.tweet_it = {
    attach: function(context) {
      var link = 'https://twitter.com/share/?url=' + Drupal.settings.nodeUrl;
      if (Drupal.settings.nodeAuthorHandlers) {
        link += '&via=' + Drupal.settings.nodeAuthorHandlersEncode;
      }
      var icon = '<span class="icon-twitter"></span>';

      $('.tweet-it').once().each(function() {
        var text = $(this).text();
        link += '&text=' + text;
        var anchor = $('<a>').attr('href', link).text(text).append(icon);
        $(this).empty().html(anchor);
      });
      $('.tweet-it-share').once().each(function() {
        link += '&text=' + Drupal.settings.nodeTitle;
        $(this).attr('href', link);
      });
    }
  }
...
;
(function($, Drupal) {
   Drupal.behaviors.chartFigures = {
    attach: function(context, settings) {
      var figures = $('.dnd-atom-wrapper.atom-figure-auto');
      var x = 1;
      figures.each(function() {
        $(this).find('.dnd-tools-figure').html('Figure ' + x); x++;
      });
    }
  }
})(jQuery, Drupal);
  ...
  Drupal.behaviors.dropDown = {
    attach: function (context) {
      $('.dropdown', context).once('dropdown').each(function() {
        // Require a target element attribute.
        if (!$(this).attr('target-element')) {
          return;
        }
        // This is the selector that we're going to use.
        var element = $(this).attr('target-element');
        $(this).click(function() {
          // Remove all other dropdown clicked classes.
          $('.dropdown').removeClass('clicked');
          
          // Only if the current div is opening should we close all visible ones.
          // Otherwise it will close itself when you click twice..
          if ($(element).is(':hidden')) {
            // TODO: this might be redundant.
            $('.clicked').removeClass('clicked');
            $(this).addClass('clicked');
            $('.dropdown-dropped:visible').slideUp(400, 'easeOutCirc').removeClass('dropdown-dropped');
          }
          $(element).slideToggle(400, 'easeOutCirc').addClass('dropdown-dropped');
        });
      });
    }
  };
