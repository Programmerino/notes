...
var RelatedOvidWidget = (function () {
    ...
    function setWidgetParams(widgetConfig) {
        ...
        //default search collection is medline
        params.searchCollection = widgetConfig.properties.searchCollection ? widgetConfig.properties.searchCollection : "medline";
    ...
    function loadWidget(options) {
        ...
        //don't do search when queryString same as previous one
        if (options.queryString === params.previousQueryString) {
            if (params.showConsoleLog) console.log("queryString equals previous queryString, so stopping here")
            return;
        }
        params.previousQueryString = options.queryString;
    ...
    function searchRelated(queryString, filterQueries) {
        ...
        xhr.onload = function () {
            if (xhr.status === 200 && xhr.readyState == 4) {
                displayResults(JSON.parse(xhr.responseText), queryString);
            }
        };
        ...
        if (params.searchCollection != 'medline') {
            request.request.Query.FilterSpecs.push({
                "FilterType": "Query",
                "FilterByQuery": {
                    "QueryString": "asset_ofType:\"article\""
                }
            });
        }
